...
> 已由显式`last-logit prefill`矩阵取代。
>
> **第二次修正：** Experiment 085发现旧cached decode把prefill已经给出的第一个token计入
> measured tokens；`new_tokens=1`因此没有执行cached Transformer forward。KV字节证据仍有效，
> 但本页cached cross-framework吞吐排名不能再作为steady decode结论。新的runner要求每个
> measured token对应一次模型forward，替代数据见Experiment 085。相同microLLM
> baseline/candidate使用同一旧协议的相对实验仍可用于候选筛选，但不能冒充服务decode速率。
