...
def validate_rocwmma_direct_bf16_model_gate(
        errors: list[str]) -> tuple[int, float, float, float, int]:
    data = REPOSITORY / "benchmarks/results/2026-08-25-rocwmma-direct-bf16-model-gate"
    summary = json.loads((data / "summary.json").read_text(encoding="utf-8"))
    verification = json.loads((data / "verification.json").read_text(
        encoding="utf-8"))
    raw = [json.loads(line) for line in (data / "raw.jsonl").read_text(
    ...
    comparisons = {(row["model"], row["case"]): row
                   for row in summary.get("comparisons", [])}
    expected = {
        ("qwen2.5-0.5b", "b1t256"): 0.8236554573657241,
        ("qwen2.5-0.5b", "b1t1024"): 0.7774515079636032,
        ("qwen2.5-0.5b", "b2t512"): 0.8881496168517451,
        ("deepseek-r1-distill-qwen-1.5b", "b1t256"): 0.8671311417411716,
        ("deepseek-r1-distill-qwen-1.5b", "b1t1024"): 0.7813177151619357,
        ("deepseek-r1-distill-qwen-1.5b", "b2t512"): 0.9062956665856382,
    }
    keys = {(row.get("model"), row.get("case"), row.get("policy"),
             int(row.get("process_run", -1))) for row in raw}
    if summary.get("schema_version") != 1 or summary.get("status") != "fail" or \
            ...
            any(row.get("schema_version") != 1 or row.get("status") != "pass" or
                ...
                (row.get("policy") == "current" and
                 int(row.get("rocwmma_online_attention_native_calls", -1)) != 0)
                for row in raw):
        errors.append("rocWMMA direct-BF16 model evidence changed")
    if verification != {
            "schema_version": 1,
            "status": "pass",
            "summary_status": "fail",
            ...
            "memory_gate": True,
            "all_top_rows_equal": True,
            "direct_bf16_boundary": True,
            "model_route_accepted": False,
            "operator_retained": True}:
        errors.append("rocWMMA direct-BF16 verification changed")
    sources = (
        ("rope_split_half_bias_bthd_bf16", REPOSITORY /
         "include/microllm/ops/ops.h"),
        ("retain_value_bf16", REPOSITORY / "src/ops/optimized.cpp"),
        ("online_bthd_attention", REPOSITORY / "src/model/model.cpp"),
    )
    if any(token not in path.read_text(encoding="utf-8")
           for token, path in sources):
        errors.append("rocWMMA direct-BF16 source contract changed")
    speeds = [float(row["speedup"]) for row in comparisons.values()]
    maximum_logit = max(float(row["maximum_absolute_logit_difference"])
                        for row in comparisons.values())
    maximum_saved = max(int(row["peak_bytes_saved"])
...
def validate_bf16_pv_output_capability(errors: list[str]) -> tuple[int, int, int]:
    root = REPOSITORY / (
        "benchmarks/results/2026-08-25-bf16-pv-output-capability")
    summary = json.loads((root / "summary.json").read_text(encoding="utf-8"))
    check = json.loads((root / "verification.json").read_text(encoding="utf-8"))
    ...
    if (summary != {
            ...
            "record_type": "bf16_pv_output_capability_summary",
            "raw_cases": 2, "supported_cases": 0, "unsupported_status": 6,
            "timed_cases": 0, "public_api_retained": False,
            "model_route_created": False,
            "decision": "reject direct BF16 P×V output on the current backend"}):
        errors.append("BF16 P×V capability summary changed")
    if (len(raw) != 2 or {row.get("case") for row in raw} != {
            "interleaved_bthd", "zero_stride_gqa_bthd"} or
            any(row.get("hipblaslt_status") != 6 or
                row.get("timing_started") is not False or
                row.get("model_route_created") is not False for row in raw)):
        errors.append("BF16 P×V raw capability evidence changed")
    if (check.get("ordinary_bthd_status") != 6 or
            check.get("gqa_bthd_status") != 6 or
            check.get("candidate_source_retained") is not False or
            check.get("model_route_created") is not False):
        errors.append("BF16 P×V verification changed")
    forbidden_tokens = (
        "attention_probability_value_bthd_bf16",
        "attention_probability_value_gqa_bthd_bf16",
    )
    public = (REPOSITORY / "include/microllm/ops/ops.h").read_text(encoding="utf-8")
    if any(token in public for token in forbidden_tokens):
        errors.append("rejected BF16 P×V API remains public")
...
def validate_current_data_parallel_audit(
        errors: list[str]) -> tuple[int, float, float, float, int]:
    root = REPOSITORY / "benchmarks/results/2026-08-25-current-data-parallel"
    summary = json.loads((root / "summary.json").read_text(encoding="utf-8"))
    audit = json.loads((root / "gap-audit.json").read_text(encoding="utf-8"))
    check = json.loads((root / "verification.json").read_text(encoding="utf-8"))
    metrics = [json.loads(line) for line in (root / "metrics.jsonl").read_text(
        encoding="utf-8").splitlines() if line]
    trace = [json.loads(line) for line in (root / "trace.jsonl").read_text(
        encoding="utf-8").splitlines() if line]
    expected = (0.35, 2.29, 0.305, 14)
    if (summary.get("schema_version") != 1 or summary.get("status") != "pass" or
            summary.get("steps") != 20 or summary.get("final_loss") != 0.55 or
            summary.get("maximum_parameter_difference") != 0.0 or
            abs(float(summary.get("median_communication_ms", 0.0)) - expected[0]) > 1.0e-12 or
            abs(float(summary.get("median_total_ms", 0.0)) - expected[1]) > 1.0e-12 or
            audit.get("record_type") != "current_data_parallel_gap_audit" or
            abs(float(audit.get("unattributed_verification_ms", 0.0)) -
                expected[2]) > 1.0e-12 or
            audit.get("bucket_count") != 1 or
            audit.get("complete_backward_sync_before_communication") is not True or
            audit.get("real_gradient_ready_overlap") is not False or
            audit.get("one_process_per_gpu") is not False or
            audit.get("four_rank_environment_blocked") is not True):
        errors.append("current data-parallel gap audit changed")
    if (len(metrics) != 20 or len(trace) != 240 or
            any(row.get("bucket_count") != 1 or
                row.get("parameter_max_difference") != 0.0 for row in metrics)):
        errors.append("current data-parallel raw evidence changed")
    if (check.get("rccl_label") != {"passed": 14, "total": 14} or
            check.get("metric_records") != 20 or check.get("trace_records") != 240 or
            check.get("maximum_parameter_difference") != 0.0 or
            check.get("four_rank_measured") is not False or
            check.get("synthetic_overlap_is_not_model_overlap") is not True):
...
def validate_data_parallel_model_s_bucket_matrix(
        errors: list[str]) -> tuple[int, int, float, float, int]:
    root = REPOSITORY / (
        "benchmarks/results/2026-08-25-data-parallel-model-s-bucket-matrix")
    summary = json.loads((root / "summary.json").read_text(encoding="utf-8"))
    check = json.loads((root / "verification.json").read_text(encoding="utf-8"))
    raw = [json.loads(line) for line in (root / "raw.jsonl").read_text(
        encoding="utf-8").splitlines() if line]
    processes = [json.loads(line) for line in (root / "process-summary.jsonl").read_text(
        encoding="utf-8").splitlines() if line]
    policies = summary.get("policies", {})
    best = policies.get("25mib", {})
    four = policies.get("4mib", {})
    expected = (3, 19.76, 6.824999999999999, 54294528)
    if (summary.get("schema_version") != 1 or summary.get("status") != "pass" or
            summary.get("record_type") !=
                "data_parallel_model_s_bucket_matrix_summary" or
            summary.get("model") != "model-s" or
            summary.get("parameter_count") != 15586176 or
            summary.get("raw_records") != 45 or summary.get("processes") != 9 or
            summary.get("loss_trajectories_exact") is not True or
            summary.get("best_policy") != "25mib" or
            best.get("bucket_count") != expected[0] or
            abs(float(best.get("median_total_ms", 0.0)) - expected[1]) > 1.0e-12 or
            abs(float(best.get("median_communication_ms", 0.0)) - expected[2]) > 1.0e-12 or
            int(best.get("maximum_engine_peak_bytes", 0)) -
                int(four.get("maximum_engine_peak_bytes", 0)) != expected[3]):
        errors.append("data-parallel Model-S bucket summary changed")
    if (len(raw) != 45 or len(processes) != 9 or any(
            row.get("parameter_count") != 15586176 or
            row.get("bucket_total_elements") != 15586176 or
            row.get("bucket_parameter_count") != 57 or
            row.get("maximum_engine_peak_bytes", 0) <= 0 for row in raw)):
        errors.append("data-parallel Model-S raw evidence changed")
    if (check.get("raw_records") != 45 or check.get("process_records") != 9 or
            check.get("loss_values_exact") != 45 or
            check.get("final_parameter_checks") != 9 or
            check.get("maximum_parameter_difference") != 0.0 or
            check.get("best_policy") != "25mib" or
            check.get("best_bucket_count") != 3 or
            check.get("peak_byte_tradeoff") != expected[3] or
            check.get("registered_test_files") != 115):
        errors.append("data-parallel Model-S verification changed")
    return summary.get("processes", 0), *expected
...
def validate_ranked_gradient_views(
        errors: list[str]) -> tuple[int, float, float, int, int, int]:
    root = REPOSITORY / "benchmarks/results/2026-08-25-ranked-gradient-views"
    summary = json.loads((root / "summary.json").read_text(encoding="utf-8"))
    check = json.loads((root / "verification.json").read_text(encoding="utf-8"))
    failure = json.loads((root / "failure.json").read_text(encoding="utf-8"))
    raw = [json.loads(line) for line in (root / "raw.jsonl").read_text(
    ...
    transient = policies.get("bucket", {})
    persistent = policies.get("persistent-bucket", {})
    views = policies.get("bucket-views", {})
    expected_maximum = 0.00627149641514
    expected_rms = 3.70091947467e-06
    expected_loss = 1.9672499999678905e-05
    expected_reducer_parameter = 0.9839284372652142
    expected_reducer_persistent = 1.1197043576258454
    expected_reducer_transient = 1.7625674305033807
    expected_training_parameter = 1.0546901474224337
    expected_training_persistent = 1.006090015684115
    expected_training_transient = 1.2736295144306116
    if (summary.get("schema_version") != 1 or summary.get("status") != "pass" or
            summary.get("record_type") != "ranked_training_matrix_summary" or
            summary.get("model") != "model-s" or
            summary.get("selected_policies") !=
                ["per-parameter", "bucket", "persistent-bucket", "bucket-views"] or
            summary.get("runs_per_policy") != 3 or
            summary.get("policy_runs") != 12 or
            ...
            transient.get("collectives_per_rank") != 9 or
            persistent.get("collectives_per_rank") != 9 or
            views.get("collectives_per_rank") != 9 or
            views.get("persistent_storage") is not True or
            views.get("plan_reuses_per_rank") != 2 or
            views.get("plan_capacity_bytes_per_rank") != 62344704 or
            views.get("gradient_views_per_rank") != 171 or
            views.get("median_steady_unpack_copies") != 0.0 or
            views.get("maximum_steady_reducer_backend_allocation_calls") != 0 or
            summary.get("views_steady_reducer_speedup_vs_per_parameter") !=
                expected_reducer_parameter or
            summary.get("views_steady_reducer_speedup_vs_persistent_copy") !=
                expected_reducer_persistent or
            summary.get("views_steady_reducer_speedup_vs_transient") !=
                expected_reducer_transient or
            summary.get("views_steady_training_speedup_vs_per_parameter") !=
                expected_training_parameter or
            summary.get("views_steady_training_speedup_vs_persistent_copy") !=
                expected_training_persistent or
            summary.get("views_steady_training_speedup_vs_transient") !=
                expected_training_transient or
            summary.get("views_current_bytes_added_vs_per_parameter") != 0 or
            summary.get("views_current_bytes_added_vs_persistent_copy") !=
                -62344704 or
            summary.get("views_peak_bytes_added_vs_per_parameter") != 62344704 or
            summary.get("views_peak_bytes_added_vs_persistent_copy") !=
                -62344704 or
            summary.get("decision") != "measure ranked gradient bucket views" or
            summary.get("peer_failure_detected") is not True or
            summary.get("peer_processes_terminated") != 1 or
            summary.get("failure_returncodes") != [1, -15]):
        errors.append("ranked gradient view summary changed")
    view_rows = [row for row in raw if row.get("reducer") == "bucket-views"]
    if (len(raw) != 12 or len(view_rows) != 3 or
            any(row.get("maximum_rank_difference") != 0.0 or
                row.get("rank_rms_difference") != 0.0 or
                row.get("maximum_reference_difference") != expected_maximum or
                row.get("reference_rms_difference") != expected_rms or
            ...
            any(row.get("maximum_rank_step_collectives") != [3, 3, 3] or
                ...
                row.get("maximum_rank_step_reducer_total_allocated_bytes") !=
                    [62344704, 0, 0] or
                row.get("maximum_rank_step_plan_reused") != [0, 1, 1] or
                row.get("maximum_rank_step_gradient_views") != [57, 57, 57] or
                row.get("maximum_rank_step_pack_copies") != [57, 57, 57] or
                row.get("maximum_rank_step_unpack_copies") != [0, 0, 0] or
                row.get("plan_capacity_bytes_per_rank") != 62344704 or
                row.get("persistent_storage") is not True
                for row in view_rows)):
        errors.append("ranked gradient view raw evidence changed")
    stdout_records = []
    for path in sorted(root.glob("run-*/*.stdout")):
        if path.name.startswith("rank"):
            stdout_records.append(json.loads(path.read_text(encoding="utf-8")))
    if (len(stdout_records) != 24 or
            ...
            list(root.rglob("*.safetensors")) or
            list(root.rglob("communicator.id"))):
        errors.append("ranked view temporary parameter evidence changed")
    if (failure.get("failure_detected") is not True or
            failure.get("peer_processes_terminated") != 1 or
            failure.get("returncodes") != [1, -15]):
        errors.append("ranked gradient view failure evidence changed")
    if (check.get("measurement_commit") !=
            "490db1a5266faf5229a42b059fc155949073fb5e" or
            check.get("dirty_at_measurement") is not False or
            check.get("gpu") != "AMD Instinct MI300X VF" or
            check.get("architecture") != "gfx942" or
            check.get("runs_per_policy") != 3 or
            check.get("selected_policies") !=
                ["per-parameter", "bucket", "persistent-bucket", "bucket-views"] or
            check.get("policy_runs") != 12 or
            check.get("rank_processes") != 24 or
            check.get("steps_per_rank") != 3 or
            check.get("steady_samples_per_policy") != 6 or
            check.get("parameter_tensors") != 57 or
            check.get("parameter_values") != 15586176 or
            check.get("views_collectives_per_rank") != 9 or
            check.get("views_gradient_views_per_rank") != 171 or
            check.get("views_plan_reuses_per_rank") != 2 or
            check.get("views_plan_capacity_bytes_per_rank") != 62344704 or
            check.get("views_later_backend_allocation_calls") != 0 or
            check.get("views_steady_pack_copies") != 57 or
            check.get("views_steady_unpack_copies") != 0 or
            check.get("views_reducer_speedup_vs_per_parameter") !=
                expected_reducer_parameter or
            check.get("views_reducer_speedup_vs_persistent_copy") !=
                expected_reducer_persistent or
            check.get("views_reducer_speedup_vs_transient") !=
                expected_reducer_transient or
            check.get("views_training_speedup_vs_per_parameter") !=
                expected_training_parameter or
            check.get("views_training_speedup_vs_persistent_copy") !=
                expected_training_persistent or
            check.get("views_training_speedup_vs_transient") !=
                expected_training_transient or
            check.get("views_current_added_vs_per_parameter") != 0 or
            check.get("views_current_added_vs_persistent_copy") != -62344704 or
            check.get("views_peak_added_vs_per_parameter") != 62344704 or
            check.get("views_peak_added_vs_persistent_copy") != -62344704 or
            check.get("maximum_rank_difference") != 0.0 or
            check.get("maximum_rank_rms_difference") != 0.0 or
            check.get("maximum_reference_difference") != expected_maximum or
            check.get("maximum_reference_rms_difference") != expected_rms or
            check.get("maximum_mean_loss_difference") != expected_loss or
            check.get("temporary_parameter_files_retained") is not False or
            check.get("bucket_views_kept_explicit") is not True or
            check.get("default_bucket_views") is not False or
            check.get("ranked_ready_overlap_admitted") is not True or
            check.get("rccl_label") != {"passed": 45, "total": 45} or
            check.get("ranked_contract") != {"passed": 5, "total": 5} or
            check.get("registered_test_files") != 123):
        errors.append("ranked gradient view verification changed")
    header = (REPOSITORY /
              "include/microllm/multi_gpu/gradient_bucket.h").read_text(
                  encoding="utf-8")
    worker = (REPOSITORY / "apps/distributed_rank.cpp").read_text(
        encoding="utf-8")
    if ("bool gradient_views = false" not in header or
            'options.reducer == "bucket-views"' not in worker or
            "step_gradient_views" not in worker):
        errors.append("ranked gradient view route is missing")
    return summary.get("policy_runs", 0), expected_reducer_parameter, \
        expected_training_parameter, \
        int(summary.get("views_plan_capacity_bytes_per_rank", 0)), \
        int(summary.get("views_current_bytes_added_vs_per_parameter", 0)), \
        int(summary.get("views_peak_bytes_added_vs_per_parameter", 0))


def validate_ranked_gradient_overlap(
        errors: list[str]) -> tuple[int, float, float, int, int]:
    root = REPOSITORY / "benchmarks/results/2026-08-25-ranked-gradient-overlap"
    summary = json.loads((root / "summary.json").read_text(encoding="utf-8"))
    check = json.loads((root / "verification.json").read_text(encoding="utf-8"))
    failure = json.loads((root / "failure.json").read_text(encoding="utf-8"))
    raw = [json.loads(line) for line in (root / "raw.jsonl").read_text(
    ...
    overlap = policies.get("overlap-views", {})
    expected_maximum = 0.00627149641514
    expected_rms = 3.70091947467e-06
    expected_loss = 1.9672499999678905e-05
    expected_finish = 2.1799837234400155
    expected_training = 1.005238966033444
    if (summary.get("schema_version") != 1 or summary.get("status") != "pass" or
            summary.get("record_type") != "ranked_training_matrix_summary" or
            summary.get("model") != "model-s" or
            summary.get("selected_policies") !=
                ["per-parameter", "bucket", "persistent-bucket", "bucket-views",
                 "overlap-views"] or
            summary.get("runs_per_policy") != 3 or
            summary.get("policy_runs") != 15 or
            summary.get("rank_processes") != 30 or
            summary.get("steps_per_rank") != 3 or
            summary.get("steady_skip_steps") != 1 or
            summary.get("steady_steps_per_run") != 2 or
            summary.get("parameter_tensors") != 57 or
            ...
            summary.get("maximum_reference_rms_difference") != expected_rms or
            summary.get("maximum_mean_loss_difference") != expected_loss or
            synchronous.get("steady_step_samples") != 6 or
            overlap.get("steady_step_samples") != 6 or
            overlap.get("overlap_steps_per_rank") != 2 or
            overlap.get("overlapped_buckets_per_rank") != 6 or
            overlap.get("maximum_steady_reducer_backend_allocation_calls") != 0 or
            summary.get("overlap_steady_finish_speedup_vs_synchronous_views") !=
                expected_finish or
            summary.get("overlap_steady_training_speedup_vs_synchronous_views") !=
                expected_training or
            summary.get("overlap_current_bytes_added_vs_synchronous_views") != 0 or
            summary.get("overlap_peak_bytes_added_vs_synchronous_views") != 0 or
            summary.get("decision") != "measure ranked gradient-ready overlap" or
            summary.get("peer_failure_detected") is not True or
            summary.get("peer_processes_terminated") != 1 or
            summary.get("failure_returncodes") != [1, -15]):
        errors.append("ranked gradient overlap summary changed")
    overlap_rows = [row for row in raw if row.get("reducer") == "overlap-views"]
    if (len(raw) != 15 or len(overlap_rows) != 3 or
            any(row.get("maximum_rank_difference") != 0.0 or
                row.get("rank_rms_difference") != 0.0 or
                row.get("maximum_reference_difference") != expected_maximum or
                row.get("reference_rms_difference") != expected_rms or
            ...
            any(row.get("maximum_rank_step_collectives") != [3, 3, 3] or
                ...
                row.get("maximum_rank_step_overlapped_buckets") != [0, 3, 3] or
                row.get("maximum_rank_step_reducer_backend_allocation_calls") !=
                    [3, 0, 0] or
                row.get("maximum_rank_step_reducer_total_allocated_bytes") !=
                    [62344704, 0, 0] or
                row.get("maximum_rank_step_gradient_views") != [57, 57, 57] or
                row.get("maximum_rank_step_unpack_copies") != [0, 0, 0] or
                row.get("overlap_steps_per_rank") != 2 or
                row.get("overlapped_buckets_per_rank") != 6
                for row in overlap_rows)):
        errors.append("ranked gradient overlap raw evidence changed")
    stdout_records = []
    for path in sorted(root.glob("run-*/*.stdout")):
        if path.name.startswith("rank"):
            stdout_records.append(json.loads(path.read_text(encoding="utf-8")))
    if (len(stdout_records) != 30 or
            any("parameters" in row or row.get("parameter_count") != 15586176
                for row in stdout_records) or
            list(root.rglob("*.safetensors")) or
            list(root.rglob("communicator.id"))):
        errors.append("ranked overlap temporary parameter evidence changed")
    if (failure.get("failure_detected") is not True or
            failure.get("peer_processes_terminated") != 1 or
            failure.get("returncodes") != [1, -15]):
        errors.append("ranked gradient overlap failure evidence changed")
    if (check.get("measurement_commit") !=
            "3320c43baf11a7c83ed3a9a423edeea7021662cc" or
            check.get("dirty_at_measurement") is not False or
            check.get("gpu") != "AMD Instinct MI300X VF" or
            check.get("architecture") != "gfx942" or
            check.get("runs_per_policy") != 3 or
            check.get("selected_policies") !=
                ["per-parameter", "bucket", "persistent-bucket", "bucket-views",
                 "overlap-views"] or
            check.get("policy_runs") != 15 or
            check.get("rank_processes") != 30 or
            check.get("steps_per_rank") != 3 or
            check.get("steady_samples_per_policy") != 6 or
            check.get("parameter_tensors") != 57 or
            check.get("parameter_values") != 15586176 or
            check.get("overlap_steps_per_rank") != 2 or
            check.get("overlapped_buckets_per_rank") != 6 or
            check.get("overlap_later_backend_allocation_calls") != 0 or
            check.get("synchronous_forward_backward_ms") != 5.256893 or
            check.get("overlap_forward_backward_enqueue_ms") != 6.456281 or
            check.get("overlap_forward_backward_added_ms") != 1.199388 or
            check.get("synchronous_finish_ms") != 3.0804805 or
            check.get("overlap_finish_ms") != 1.413075 or
            check.get("overlap_finish_speedup") != expected_finish or
            check.get("synchronous_steady_training_ms") != 8.195082 or
            check.get("overlap_steady_training_ms") != 8.152372 or
            check.get("overlap_training_speedup_vs_synchronous_views") !=
                expected_training or
            check.get("minimum_required_speedup") != 1.01 or
            check.get("performance_gate_passed") is not False or
            check.get("overlap_current_added") != 0 or
            check.get("overlap_peak_added") != 0 or
            check.get("maximum_rank_difference") != 0.0 or
            check.get("maximum_rank_rms_difference") != 0.0 or
            check.get("maximum_reference_difference") != expected_maximum or
            check.get("maximum_reference_rms_difference") != expected_rms or
            check.get("maximum_mean_loss_difference") != expected_loss or
            check.get("temporary_parameter_files_retained") is not False or
            check.get("overlap_route_kept_explicit") is not True or
            check.get("default_overlap") is not False or
            check.get("model_s_t32_local_overlap_search_closed") is not True or
            check.get("rccl_label") != {"passed": 47, "total": 47} or
            check.get("ranked_contract") != {"passed": 5, "total": 5} or
            check.get("registered_test_files") != 123):
        errors.append("ranked gradient overlap verification changed")
    header = (REPOSITORY /
              "include/microllm/multi_gpu/gradient_bucket.h").read_text(
                  encoding="utf-8")
    worker = (REPOSITORY / "apps/distributed_rank.cpp").read_text(
        encoding="utf-8")
    if ("begin_overlap_step" not in header or
            'options.reducer == "overlap-views"' not in worker or
            "step_overlapped_buckets" not in worker):
        errors.append("ranked gradient overlap route is missing")
    return summary.get("policy_runs", 0), expected_finish, expected_training, \
        int(summary.get("overlap_current_bytes_added_vs_synchronous_views", 0)), \
...
def main() -> int:
    ...
    data_parallel_model_s_processes, data_parallel_model_s_buckets, \
    ...
    ranked_view_runs, ranked_view_reducer, ranked_view_training, \
        ranked_view_capacity, ranked_view_current, ranked_view_peak = \
        validate_ranked_gradient_views(errors)
    ...
    print(f"optimization log valid: results={result_count} steps={step_count} "
          ...
          f"ranked_views={ranked_view_runs}/"
          f"{ranked_view_reducer:.4f}/"
          f"{ranked_view_training:.4f}/"
          f"{ranked_view_capacity}/"
          f"{ranked_view_current}/"
          f"{ranked_view_peak} "
          f"ranked_overlap={ranked_overlap_runs}/"
          f"{ranked_overlap_finish:.4f}/"
