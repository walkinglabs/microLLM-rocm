...
| M4 low precision | in progress | Qwen/DeepSeek matrices；streaming load；optimizer saturated；T=512 profile；scalar split backward discarded | tiled/flash backward；multi-shard preflight |
...
| [08](steps/08-batched-fmha.md) | in progress | cached/full causal GQA fused；T=512 atomics now dominant | long-sequence backward + memory |
