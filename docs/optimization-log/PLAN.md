...
| M4 low precision | in progress | Qwen/DeepSeek four-shape matrices；device-native load；retained optimizer profile | stable gradient buffers；multi-tensor AdamW；streaming/mapped load；T=512 |
...
| [07](steps/07-autograd-buffers.md) | in progress | local COW add_ discarded；Experiment 046 selects stable gradient addresses for multi-tensor AdamW | train allocations / optimizer launches |
