...
| Serving scheduler | smoke-tested | refill, official S1–S8 matrix, full-row fix and opt-in source/batch/top-2 diagnostics | DeepSeek low-margin divergence isolated to prefill shape; B2 row/copy audit, length-aware Cache and latency |
...
| Optimization experiment journal | implemented | 104 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
