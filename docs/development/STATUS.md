...
|---|---|---|---|
| CPU configuration | smoke-tested | CPU 332/332; full CPU/HIP 524/524 with 3 conditional skips; HIP label 180/180; ASan/UBSan 330/330 | broader compiler/OS CI matrix |
...
| SGD/AdamW | smoke-tested | PyTorch parity through standard step 3 and BF16-moment step 32; 100-step rounded CPU reference; immutable-descriptor two-node Graph aligns 53-step FP32/BF16 state and rescues many-small Tensor submission | stable gradient addresses before model Graph routing, loss scaling, other-GPU tuning and reference-length training |
...
| PyTorch correctness oracle | smoke-tested | PyTorch-enabled build 306/306; Tensor/graph/model/optimizer parity plus package and schema gates | broader direct PyTorch ROCm operator matrix |
...
| Optimization experiment journal | implemented | experiments through 222; raw evidence, rejection gates and generated SVGs validated in CTest | Radeon and production data-parallel tracks |
