...
| Serving scheduler | smoke-tested | continuous refill/compaction/positions-aware decode, request TTFT, shared-weight length buckets, logical arrivals and focus P95; skew/delayed MI300X matrix is 36/36 token-exact | compatible short-request overflow; long-heavy still needs adaptive slots or paged Cache |
...
| Optimization experiment journal | implemented | 116 experiments; skewed traffic tail failure plus two retained device-gate blocks added | HIP Graph, Radeon and production data-parallel tracks |
