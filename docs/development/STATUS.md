...
|---|---|---|---|
| CPU configuration | smoke-tested | CPU 340/340; full CPU/HIP 536/536 with 3 conditional skips; HIP label 184/184; ASan/UBSan 338/338 | broader compiler/OS CI matrix |
...
| PyTorch correctness oracle | smoke-tested | PyTorch-enabled build 314/314; Tensor/graph/model/optimizer parity plus package and schema gates | broader direct PyTorch ROCm operator matrix |
...
| Optimization experiment journal | implemented | experiments through 229; raw evidence, rejection gates and generated SVGs validated in CTest | Radeon and production data-parallel tracks |
...
| Public online Attention operator | smoke-tested | BF16→FP32 BTHD API, gfx942 batch-native route, exact counters, PyTorch/CPU/HIP; 10 native cases 1.534×–2.456× and 4 exact fallback counterexamples | Qwen/DeepSeek full-logit, peak-memory and end-to-end model A/B; default model route off |
