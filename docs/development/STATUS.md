...
|---|---|---|---|
| CPU configuration | smoke-tested | full 265/265 CPU/HIP configuration includes 189 CPU-labelled and 76 HIP-labelled gates; ASan/UBSan 182/182 | broader compiler/OS CI matrix |
...
| CPU/HIP KV cache | smoke-tested | batch-aware FP32/BF16 request Storage, full prefill/step, B2 T4097 fallback, complete-logit diagnostics and exact 2× BF16 byte reduction | DeepSeek T512 B1 BF16 RMSE failure and request scheduling |
...
| PyTorch correctness oracle | smoke-tested | PyTorch-enabled 187/187; 30+ Tensor APIs, 29 graph APIs, inference matrix and model/optimizer parity | broader direct PyTorch ROCm operator matrix |
...
| Optimization experiment journal | implemented | 66 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
