...
|---|---|---|---|
| CPU configuration | smoke-tested | CPU 333/333; full CPU/HIP 526/526 with 3 conditional skips; HIP label 181/181; ASan/UBSan 331/331 | broader compiler/OS CI matrix |
| CPU code coverage | smoke-tested | 78.5% lines, 86.8% functions, 59.2% branches over `src/` + `include/`; immutable-descriptor Graph and other HIP-only paths remain visible as CPU gaps | split CPU/HIP reports and add justified thresholds |
...
| SGD/AdamW | smoke-tested | PyTorch parity through standard step 3 and BF16-moment step 32; immutable two-node Graph aligns 53-step FP32/BF16 state; address audit proves Qwen T8/T512 and DeepSeek T8 stable but DeepSeek T512 changes 198 gradients/7.108GB | optimizer-phase model gate only for snapshot-proven cases; stable gradient design for DeepSeek long context |
...
| PyTorch correctness oracle | smoke-tested | PyTorch-enabled build 307/307; Tensor/graph/model/optimizer parity plus package and schema gates | broader direct PyTorch ROCm operator matrix |
...
| Optimization experiment journal | implemented | experiments through 223; raw evidence, rejection gates and generated SVGs validated in CTest | Radeon and production data-parallel tracks |
