...
| Serving scheduler | smoke-tested | delayed/static/admission, continuous refill, active compaction, positions-aware decode, packed metadata and equal-length batched prefill | uniform R8/S8 reaches 87.4% static; mixed-length packing, persistent state and real-model serving matrix |
...
| Optimization experiment journal | implemented | 101 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
