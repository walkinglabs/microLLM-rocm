...
|---|---|---|---|
| CPU configuration | smoke-tested | full 314/314 configuration includes 218 CPU-labelled and 96 HIP-labelled gates; ASan/UBSan 211/211 | broader compiler/OS CI matrix |
...
| Serving scheduler | smoke-tested | delayed/static/admission, continuous refill, active compaction, positions-aware decode, packed metadata, batched prefill and official short/long 2/4-slot matrix | Qwen exact 4/4; DeepSeek only 1/4, then fixed-request 1/2/4/8 sweep, latency and persistent state |
...
| Profiling/autotune | smoke-tested | rocprofv3, hipBLASLt, exact-shape registry, DeepSeek T2048 audit and continuous-only scheduler mode with raw pftrace | persistent arch/version candidate cache and official-model serving trace |
...
| Optimization experiment journal | implemented | 102 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
