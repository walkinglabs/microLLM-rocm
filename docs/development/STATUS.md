...
|---|---|---|---|
| CPU configuration | smoke-tested | full CPU/HIP gate 370/370 with 257 CPU and 114 HIP labels; ASan/UBSan 250/250 | broader compiler/OS CI matrix |
| CPU code coverage | smoke-tested | 82.6% lines, 90.3% functions, 63.2% branches over `src/` + `include/` | split CPU/HIP reports and add justified thresholds |
...
| PyTorch correctness oracle | smoke-tested | PyTorch-enabled 226/226; 30+ Tensor APIs, 29 graph APIs, inference matrix and model/optimizer parity | broader direct PyTorch ROCm operator matrix |
...
| Matmul tuning registry | smoke-tested | exact dtype/layout/arch/version/mode/workspace key with cross-key isolation | persistent cache and correctness-before-timing autotuner |
