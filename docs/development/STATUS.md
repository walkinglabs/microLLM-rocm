...
|---|---|---|---|
| CPU configuration | smoke-tested | full 268/268 CPU/HIP configuration includes 191 CPU-labelled and 77 HIP-labelled gates; ASan/UBSan 184/184 | broader compiler/OS CI matrix |
...
| CPU/HIP KV cache | smoke-tested | batch-aware FP32/BF16/per-layer Storage, full prefill/step, B2 T4097 fallback, uniform 2× and strict 1.92×/1.931× byte reduction | strict policy portability and request scheduling |
...
| PyTorch correctness oracle | smoke-tested | PyTorch-enabled 189/189; 30+ Tensor APIs, 29 graph APIs, inference matrix and model/optimizer parity | broader direct PyTorch ROCm operator matrix |
...
| Optimization experiment journal | implemented | 69 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
