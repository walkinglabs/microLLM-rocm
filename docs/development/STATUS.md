...
| Serving scheduler | smoke-tested | delayed/static/admission, continuous refill, recycled full-row fix, official fixed-request 1/2/4/8 slot/KV/peak matrix | 48/48 executes; DeepSeek short cross-slot mismatch, length-aware Cache, latency and persistent state |
...
| Optimization experiment journal | implemented | 103 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
