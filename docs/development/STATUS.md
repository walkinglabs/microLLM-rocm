...
|---|---|---|---|
| Current validation configurations | smoke-tested | CPU 367/367, ASan/UBSan 365/365, RCCL label 49/49; retained producer CPU/HIP/PyTorch targeted gates pass; prior single-GPU HIP label 188/188 with 1 conditional skip remains applicable | broader compiler/OS/GPU matrix |
...
| Optimization experiment journal | implemented | experiments through 277; raw evidence, rejection gates and generated SVGs validated in CTest | Radeon and production data-parallel tracks |
| Ranked Model-S uneven-input weighting | smoke-tested, synchronous | `[B1T32,B2T32]`; scales 0.6667/1.3333; 57 tensors rank-exact; CPU Max/RMS 0.007760/3.639e-6; peak 275.8MB | scale-before-ready weighted overlap ordering |
| Ranked uneven-input weighting | smoke-tested on tiny | `[B1,B2]` equal-only 2/2 fail before parameter reduce; weighted scales 0.6667/1.3333; 3-step rank exact; CPU Max/RMS 8.18e-8/8.79e-9 | Model-S sync smoke; weighted ready-overlap ordering |
...
| Ranked Model-S checkpoint | smoke-tested | 187,042,096-byte full state; 1+1 vs uninterrupted 2-step bytes equal; 57 tensors rank-exact; write 1.02–1.07s/restore 740ms; no retained files | world-size 4 interface/failure boundary; repeated storage environments |
| Ranked checkpoint ownership/resume | smoke-tested on tiny | 2+3 vs uninterrupted 5-step checkpoints byte-identical; all parameter diffs 0; rank0 writes 3/nonzero rank 0; failure [-15,1] | Model-S checkpoint size/I/O/restore smoke |
| Ranked overlap context scale | smoke-tested, context-selective | T32 total 0.9995×; T128 1.0923× and sensitivity 1.069×; finish 2.022×/2.235×; no current/peak delta; full correctness passes | other models/world sizes/buckets; rank checkpoint ownership |
| Ranked gradient-ready overlap | implemented, explicit; T32 performance rejected | finish wait 2.180×, total 1.0052×; 3 buckets × 2 steady steps; no allocation/current/peak increase; full correctness passes | separate T128+ scale track or close Model-S overlap |
