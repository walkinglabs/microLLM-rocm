...
|---|---|---|---|
| CPU configuration | smoke-tested | full 306/306 configuration includes 214 CPU-labelled and 92 HIP-labelled gates; ASan/UBSan 207/207 | broader compiler/OS CI matrix |
...
| Serving scheduler | smoke-tested | delayed/static/admission plus fixed-row continuous refill, independent RNG/stop/cancel, FP32/BF16 CPU/HIP and slot/KV metrics | Release divergent path is 0.748x–0.858x serial; parallel unequal-position kernels, active compaction and batched prefill |
...
| Optimization experiment journal | implemented | 96 numbered FP32/BF16/load/training/inference experiments with generated figures, raw evidence and keep/discard/invalid gates | HIP Graph, Radeon and production data-parallel tracks |
