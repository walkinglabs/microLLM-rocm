...
The rejected model route used optional loss and gate/up-parameter output paths on the training
CLI. Its candidate-only runner was removed, while the generic interfaces remain. Both files are
written after the measured window. `microllm_compare_safetensors` compares every FP32 master
