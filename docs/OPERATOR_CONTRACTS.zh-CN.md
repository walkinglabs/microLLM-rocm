...
## MoE 路由（CPU/HIP readable 前向 + CPU backward 已实现；ROCm 硬件门尚未跑）
...
要等有 AMD GPU 时才能真正跑，见 M2 记录里的说明。M3 的 backward 目前也是 CPU-only（HIP backward
显式 `throw`），已经用 conda 环境的 CUDA 版 PyTorch 跑过真实的 `TorchOps.OperatorParity` 三方门。
