...
> External integration now has an explicit leaf-gradient destination. The 18-process MI300X model
> gate keeps all 21 Tiny and 57 Model-S addresses stable and makes every complete gradient bit-exact.
> It is deliberately not the default: Event is only 0.792×–0.871× and Model-S measured peak grows
> 6.75–10.69MiB. Stable addresses remain useful for foreign runtimes and communication buffers.
...
> copies using three 1,368-byte-described gather-scale launches. Its full T128
> gate is correct but reaches only `1.0140x`, is 0.090 ms slower than the kept
> route, and adds 1,368 bytes; it remains a research primitive, not a speed path.

> Direct FP16/BF16 Softmax now uses one block per row above width 32. The same
> ten-case PyTorch ROCm matrix stays precision/pointer/zero-temporary clean;
> width128/1024 reaches `1.213×–1.252×`/`1.103×–1.114×` PyTorch. Width4096 is
...
> The completed 128/256/512/1024 matrix selects 1024 threads for cached FP16 only.
> Width4096 Event falls from `12.027/7.567/5.472` to `5.086 μs`; 1024 is another
...
> That C++ Custom Op now exists for FP32/FP16/BF16 with CPU, ROCm current Stream,
> Meta/fullgraph and C++ Autograd. A no-grad gate improves FP16 width4096 by `1.158×`;
...
> Real-checkpoint layer alignment is now complete for context 4. Qwen2.5-0.5B and
> DeepSeek-Distill-1.5B expose all `27/31` embedding→block→norm→logit stages against
...
The optional PyTorch dispatcher itself is now measured on ROCm: FP32/FP16/BF16
...
An isolated backward rebuttal removes the float4 candidate: it reaches only
`0.946×–1.039×` scalar. The retained scalar fused producer is already
`2.07×–2.82×` the readable native formula with one-third lower peak, pointing instead
to Autograd's zero-stride gradient materialization.
That materialization is now removed for the exact FP32 `sum()` layout: a 4-byte device
seed is read directly. At 64K/1M, F+B improves `1.164×/1.081×` versus the old bridge and
temporary peak falls `99.42%–99.96%`; mean, weighted and general gradients retain the
ordinary path. The result remains `0.773×–0.781×` native Torch, so no training parity claim.
