...
void launch_quantize_int8_dynamic(
    const void* input, DType input_dtype, std::int8_t* output, float* scale,
    float* partial_maxima, std::int64_t partial_count,
    std::int64_t elements, float minimum_scale, void* stream = nullptr);
void launch_quantize_int8_columns_dynamic(
    const void* input, DType input_dtype, std::int8_t* output,
    float* scales, std::int64_t rows, std::int64_t columns,
    float minimum_scale, void* stream = nullptr);
void launch_dequantize_int8(const std::int8_t* input, void* output,
                            DType output_dtype, std::int64_t elements,
                            const float* scale, std::int64_t columns,
                            bool column_scales, void* stream = nullptr);
...
void launch_swiglu_backward_scalar_seed(
    ...
    float* gate_gradient, float* up_gradient,
    std::int64_t elements, void* stream = nullptr);
void launch_swiglu_backward_typed(
    const void* gate, const void* up, const void* gradient,
    void* gate_gradient, void* up_gradient, DType dtype,
    std::int64_t elements, void* stream = nullptr);
...
void launch_cached_attention_finalize_scores(
    ...
    std::int64_t sequence, std::int64_t cache_batch_stride,
    std::int64_t cache_head_stride, std::int64_t width,
    std::int64_t repeats, std::int64_t finalize_threads,
    void* stream = nullptr);
