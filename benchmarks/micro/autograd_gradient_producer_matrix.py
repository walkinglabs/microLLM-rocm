#!/usr/bin/env python3
"""Scoped Autograd caller-owned weight-gradient shape gate."""
...
def options() -> argparse.Namespace:
    ...
    if (not args.binary.is_file() or args.runs <= 0 or args.warmup < 0 or
            args.repetitions <= 0):
        parser.error("Autograd gradient producer matrix inputs are invalid")
    return args


def execute(args: argparse.Namespace, name: str, rows: int, hidden: int,
            width: int, process_run: int) -> dict:
    order = "baseline-first" if process_run % 2 else "direct-first"
    ...
    if (row.get("status") != "pass" or
            row.get("record_type") != "autograd_gradient_producer_probe" or
            row.get("rows") != rows or row.get("hidden") != hidden or
            row.get("width") != width or row.get("order") != order or
            row.get("complete_gradient_elements") != hidden * width or
        ...
        raise RuntimeError(f"{name} scoped Autograd contract changed")
    row.update({
        "shape": name,
...
def main() -> int:
    ...
    for name, rows, hidden, width, model_s_shape in SHAPES:
        ...
        baseline_event = median(records, "baseline_event_ms_p50")
        direct_event = median(records, "direct_event_ms_p50")
        baseline_wall = median(records, "baseline_wall_ms_p50")
        direct_wall = median(records, "direct_wall_ms_p50")
        event_speedup = baseline_event / direct_event
        wall_speedup = baseline_wall / direct_wall
        shapes[name] = {
            "rows": rows,
            "hidden": hidden,
            "width": width,
            ...
            "baseline_event_ms_p50": baseline_event,
            "direct_event_ms_p50": direct_event,
            "baseline_wall_ms_p50": baseline_wall,
    ...
    decision = ("admit exact right-leaf shapes to model gate"
                if admitted else
                "remove scoped Autograd producer route")
    summary = {
        "schema_version": 1,
        "status": "pass",
        "record_type": "autograd_gradient_producer_matrix_summary",
        "raw_records": len(raw),
        "runs_per_shape": args.runs,
        "warmup": args.warmup,
        "repetitions": args.repetitions,
        "aggregation": "median of fresh-process p50 with alternating operation and shape order",
        "complete_gradients_exact": True,
        "target_addresses_preserved": True,
        "allocation_calls_removed_per_invocation": 1.0,
        "direct_dispatches_per_invocation": 1.0,
        "shapes": shapes,
...
if __name__ == "__main__":
    try:
        ...
        print(f"autograd_gradient_producer_matrix: {error}", file=sys.stderr)
        raise SystemExit(2)
