...
struct Options {
    ...
    std::string implementation = "broadcast";
    int warmup = 3;
    int repetitions = 20;
};
...
Options parse(int argc, char** argv) {
    ...
    if (result.batch <= 0 || result.heads <= 0 || result.kv_heads <= 0 ||
        result.heads % result.kv_heads != 0 || result.sequence <= 0 ||
        result.width <= 0 || result.batch > 16 || result.heads > 256 ||
        result.sequence > 4096 || result.width > 512 || result.warmup < 0 ||
        result.repetitions <= 0 ||
        (result.implementation != "repeated" &&
         result.implementation != "broadcast")) {
        throw std::invalid_argument("GQA broadcast options are outside the safe contract");
    }
    return result;
}
...
microllm::Tensor execute(const Options& options,
    ...
    if (options.implementation == "broadcast") {
        return microllm::ops::attention_probability_value_gqa_bthd(
            probabilities, value, repeats);
    }
    return microllm::ops::attention_probability_value_bthd(
        probabilities,
        microllm::ops::repeat_interleave(value, 2, repeats));
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (microllm::runtime::hip_device_count() == 0 ||
            ...
            throw std::runtime_error("GQA broadcast benchmark requires HIP and hipBLASLt");
        }
        ...
        reference_options.implementation = "repeated";
        const auto reference = execute(
            reference_options, probabilities, value).to_vector();
        ...
        if (!finite || maximum_error > 3.0e-4F || rms_error > 1.0e-5) {
            throw std::runtime_error("GQA broadcast complete-output gate failed");
        }
        ...
        if (transfers.host_to_device_calls != 0 ||
            ...
            throw std::runtime_error("GQA broadcast timing performed a payload transfer");
        }
        const auto info = microllm::runtime::device_info(device);
        std::cout << std::setprecision(9)
                  << "{\"schema_version\":1,\"status\":\"pass\""
                  << ",\"op\":\"attention_probability_value_gqa_bthd\""
                  << ",\"architecture\":\"" << info.architecture << "\""
                  << ",\"batch\":" << options.batch
                  << ",\"heads\":" << options.heads
                  << ",\"kv_heads\":" << options.kv_heads
                  << ",\"repeats\":" << options.heads / options.kv_heads
        ...
        std::cerr << "benchmark_attention_gqa_broadcast: " << error.what() << '\n';
        return 2;
    }
}
