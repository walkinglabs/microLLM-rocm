#!/usr/bin/env python3
"""Locate the first block-0 drift after forcing an exact prefill Attention core."""
...
def options() -> argparse.Namespace:
    ...
    if (not args.manifest.is_file() or not args.binary.is_file() or
        ...
        parser.error("post-exact-core trace inputs are outside the contract")
...
def require_route(record: dict, batch: int) -> None:
    expected = {
        ...
        "fp32_prefill_attention_qk_solution_index": QK_SOLUTION,
        "fp32_prefill_attention_pv_solution_index": PV_SOLUTION,
...
def render(summary: dict) -> str:
    return (BASE.render(summary)
            .replace("First drift after exact block-0 cache",
                     "First drift after exact block-0 Attention core")
            .replace("Q296100 · KV292135 · complete first two batch rows",
                     "Q296100 · KV292135 · QK304681 · PV295716 · complete rows"))
...
def main() -> int:
    ...
    processes = []
    with tempfile.TemporaryDirectory(prefix="microllm-post-exact-core-") as root:
        temporary = Path(root)
        for run in range(1, args.runs + 1):
            ...
            reference = BASE.load_trace(reference_trace, 1)
            ...
            for batch in BATCHES:
                if batch == 1:
                    ...
                    actual = BASE.load_trace(trace, batch)
                    record = COMMON.last_json(current.stdout)
                    require_route(record, batch)
                processes.append({
                    "schema_version": 1,
                    "record_type": "post_exact_core_block0_trace_process",
                    "status": "pass",
                    "model": args.model,
                    ...
                    "stages": BASE.compare(reference, actual, batch),
                })
                print(json.dumps({
                ...
                if batch != 1:
                    del actual
            del reference
    summary = BASE.summarize(processes)
    summary["record_type"] = "post_exact_core_block0_trace_audit"
    summary["qk_solution_index"] = QK_SOLUTION
    summary["pv_solution_index"] = PV_SOLUTION
    (args.output_directory / "raw.jsonl").write_text(
        "".join(json.dumps(row, sort_keys=True) + "\n" for row in processes),
        encoding="utf-8")
    (args.output_directory / "summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (args.output_directory / "post-exact-core-trace.svg").write_text(
...
if __name__ == "__main__":
    try:
        ...
        print(f"audit_post_exact_core_block0_trace: {error}", file=sys.stderr)
