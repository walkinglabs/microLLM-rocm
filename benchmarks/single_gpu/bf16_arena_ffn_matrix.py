#!/usr/bin/env python3
"""Run official Qwen/DeepSeek BF16 FFN arena policies in fresh processes."""
...
def main() -> int:
    ...
    modes = ["baseline", "arena", "arena_graph"]
    raw: list[dict] = []
    ...
    for model in ("qwen", "deepseek"):
        for rows in (1, 32, 512):
            ...
            baseline = median("baseline", "wall_p50_ms")
            arena = median("arena", "wall_p50_ms")
            graph = median("arena_graph", "wall_p50_ms")
            setup = median("arena_graph", "graph_setup_ms")
            saving = baseline - graph
            comparisons.append({
                "model": model,
                "rows": rows,
                "hidden": grouped["baseline"][0]["hidden"],
                "intermediate": grouped["baseline"][0]["intermediate"],
                "baseline_wall_p50_ms": baseline,
                "arena_wall_p50_ms": arena,
                "arena_graph_wall_p50_ms": graph,
                "arena_speedup": baseline / arena,
                "arena_graph_speedup": baseline / graph,
                ...
                "baseline_allocation_calls": int(statistics.median(
                    row["measured_allocation_calls"]
                    for row in grouped["baseline"])),
                "arena_allocation_calls": int(statistics.median(
                    row["measured_allocation_calls"]
                    for row in grouped["arena"])),
                "graph_allocation_calls": int(statistics.median(
                    row["measured_allocation_calls"]
                    for row in grouped["arena_graph"])),
            })
    ...
    correctness = all(
        ...
        for row in raw)
    graph_layout = all(
        row["graph_node_count"] in (5, 6) for row in comparisons)
    eager_passes = sum(row["arena_speedup"] >= 1.05 for row in comparisons)
    graph_passes = sum(
        row["arena_graph_speedup"] >= 1.05 for row in comparisons)
    summary = {
        "schema_version": 1,
        "status": "pass" if correctness and graph_layout else "fail",
        "record_type": "bf16_arena_ffn_matrix_summary",
        "raw_processes": len(raw),
        "correctness_gate": correctness,
        "graph_layout_contract": graph_layout,
        "arena_keep_rows": eager_passes,
        "arena_graph_keep_rows": graph_passes,
        "comparisons": comparisons,
        "decision": "measure complete-model BF16 FFN arena",
    }
