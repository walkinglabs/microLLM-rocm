...
def options() -> argparse.Namespace:
    ...
    parser.add_argument("--fp8-activation-scale-mode",
                        ...
                        default="fixed")
    parser.add_argument("--fp8-activation-format",
                        choices=("e4m3-fnuz", "e5m2-fnuz"),
                        default="e4m3-fnuz")
...
def command(args: argparse.Namespace, model: dict, context: int,
    ...
    if policy == "bf16":
        ...
        activation_format = getattr(args, "fp8_activation_format", "e4m3-fnuz")
        result.extend([
            "--fp8-linear", "true",
            ...
            "--fp8-activation-format", activation_format,
...
def experiment_boundary(weight_scale_mode: str,
                        ...
                        activation_format: str = "e4m3-fnuz") -> str:
    weight_boundary = (
            ...
            for process_run in range(1, args.runs + 1):
                ...
                for policy, output, values, pre, post in pending:
                    ...
                    row = {
                        ...
                        "fp8_output_column_native_status": output.get(
                            "fp8_output_column_native_status", -1),
                        ...
                        "fp8_activation_format": args.fp8_activation_format,
        ...
        for context in args.contexts:
            for policy in ("fp32", "bf16", "fp8"):
                ...
                aggregates.append({
                    ...
                    "fp8_output_column_native_statuses": sorted(set(
                        row["fp8_output_column_native_status"] for row in selected)),
                    ...
                    "fp8_dynamic_clipped_tensor_calls_p50": statistics.median(
                        row["fp8_dynamic_clipped_tensor_calls"] for row in selected),
                })
