#!/usr/bin/env python3
"""Compare every independent official Qwen3 gradient and updated parameter."""
...
EXPECTED_ELEMENTS = 596_049_920
EXPECTED_STORED_TENSORS = 311
LIMITS = {
    "fp32": {
        "gradient_max": 1.0e-2, "gradient_rms": 1.0e-5,
...
def options() -> argparse.Namespace:
    ...
    parser.add_argument("--retain-tensors", action="store_true")
...
def parameter_family(name: str) -> str:
    ...
    if ".ffn_norm." in name:
        return "ffn_norm"
    if ".attention.q_norm." in name or ".attention.k_norm." in name:
        return "attention_qk_norm"
    if any(f".attention.{projection}." in name
           for projection in ("q_proj", "k_proj", "v_proj")):
        return "attention_qkv"
    if ".attention.o_proj." in name:
        return "attention_output"
    if (".feed_forward.gate_proj." in name or
            ".feed_forward.up_proj." in name):
        return "ffn_gate_up"
    if ".feed_forward.down_proj." in name:
        return "ffn_down"
    raise RuntimeError(f"unclassified internal parameter: {name}")


def compare_files(candidate_path: Path, reference_path: Path,
                  kind: str) -> tuple[list[dict], dict, list[dict]]:
    records = []
    total_elements = 0
    total_squared = 0.0
    maximum = 0.0
    worst_tensor = ""
    maximum_tensor_rms = 0.0
    worst_rms_tensor = ""
    ...
    with safe_open(candidate_path, framework="pt", device="cpu") as candidate, \
        ...
        if candidate_names != reference_names or len(candidate_names) != EXPECTED_TENSORS:
            ...
            raise RuntimeError(
                f"all-parameter exports disagree: missing={missing}, extra={extra}")
        if ("token_embedding.weight" not in candidate_names or
                "output_head.weight" in candidate_names or
                "lm_head.weight" in candidate_names):
            raise RuntimeError("tied embedding/output alias contract failed")
        for name in sorted(candidate_names):
            left = candidate.get_tensor(name).float()
            right = reference.get_tensor(name).float()
            if left.shape != right.shape or left.numel() == 0:
                raise RuntimeError(f"all-parameter shape mismatch: {name}")
            ...
            records.append({
                "schema_version": 1,
                "record_type": "qwen3_training_parameter_tensor_comparison",
                "status": "pass" if finite else "nonfinite", "kind": kind,
                "family": family, "name": name, "shape": list(left.shape),
                "elements": left.numel(),
            ...
            group = grouped.setdefault(family, {
                "tensor_count": 0, "compared_elements": 0,
                "maximum_absolute_difference": 0.0,
                "squared_difference": 0.0, "worst_tensor": "",
                "all_finite": True,
            })
            group["tensor_count"] += 1
            group["compared_elements"] += difference.numel()
            group["squared_difference"] += squared
            ...
            if tensor_max > maximum:
                maximum = tensor_max
                worst_tensor = name
            if tensor_rms > maximum_tensor_rms:
                maximum_tensor_rms = tensor_rms
                worst_rms_tensor = name
    ...
    if total_elements != EXPECTED_ELEMENTS or not all(
        ...
        raise RuntimeError("all-parameter element/finite contract failed")
    family_records = []
    for family, group in sorted(grouped.items()):
        squared = group.pop("squared_difference")
        group["rms_difference"] = math.sqrt(
            squared / group["compared_elements"])
        family_records.append({"family": family, **group})
    return records, {
        ...
        "maximum_tensor_rms_difference": maximum_tensor_rms,
        "worst_rms_tensor": worst_rms_tensor,
    }, family_records
...
def main() -> int:
    ...
    if (model.get("runtime_parameter_count") != EXPECTED_ELEMENTS or
            model.get("loaded_tensors") != EXPECTED_STORED_TENSORS):
        raise RuntimeError("manifest does not match the declared Qwen3 alias contract")
    ...
    micro_command = [
        ...
        "--all-gradients-output", str(micro_grad),
        "--all-parameters-output", str(micro_param),
    ]
    pytorch_command = [
        ...
        "--all-gradients-output", str(torch_grad),
        "--all-parameters-output", str(torch_param),
    ]
    ...
    for framework, record in (("microllm", micro), ("pytorch", pytorch)):
        if (record.get("status") != "pass" or
                record.get("all_gradient_tensors") != EXPECTED_TENSORS or
                record.get("all_gradient_elements") != EXPECTED_ELEMENTS or
                record.get("all_parameter_tensors") != EXPECTED_TENSORS or
                record.get("all_parameter_elements") != EXPECTED_ELEMENTS):
            raise RuntimeError(f"{framework} all-parameter export contract failed")
    gradient_records, gradients, gradient_families = compare_files(
        micro_grad, torch_grad, "gradient")
    parameter_records, parameters, parameter_families = compare_files(
        micro_param, torch_param, "parameter")
    limits = LIMITS[args.precision]
    gates = {
        "loss_finite": math.isfinite(float(micro["loss"])) and
                       math.isfinite(float(pytorch["loss"])),
        "tied_alias": micro["all_parameter_tensors"] == EXPECTED_TENSORS and
                      pytorch["all_parameter_tensors"] == EXPECTED_TENSORS,
    ...
    export_files = (micro_grad, micro_param, torch_grad, torch_param)
    export_bytes = sum(path.stat().st_size for path in export_files)
    exports_removed = False
    if not args.retain_tensors:
        for path in export_files:
            path.unlink()
        exports_removed = all(not path.exists() for path in export_files)
    summary = {
        "schema_version": 1,
        "record_type": "qwen3_training_all_parameter_audit",
        ...
        "source_stored_tensors": EXPECTED_STORED_TENSORS,
        "independent_runtime_tensors": EXPECTED_TENSORS,
        "independent_runtime_elements": EXPECTED_ELEMENTS,
        "tied_alias": "lm_head.weight -> token_embedding.weight",
        "microllm_loss": float(micro["loss"]),
        "pytorch_loss": float(pytorch["loss"]),
        "absolute_loss_difference": abs(float(micro["loss"]) -
                                        float(pytorch["loss"])),
        "gradients": gradients, "gradient_families": gradient_families,
        "parameters": parameters, "parameter_families": parameter_families,
        ...
        "temporary_exports_retained": args.retain_tensors,
        "temporary_exports_removed": exports_removed,
        "boundary": (
            "all 310 independent runtime gradients before AdamW and parameters "
            "after one step; AdamW moments and multi-step trajectory are separate"),
    }
